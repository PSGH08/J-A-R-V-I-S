// Core.jsx (Awake state - animated center with J.A.R.V.I.S text)
import { motion } from "framer-motion";

export default function Core() {
  return (
    <motion.div
      animate={{
        scale: [1, 1.03, 1],
      }}
      transition={{
        scale: {
          duration: 4,
          repeat: Infinity,
        },
      }}
      className="relative flex h-[200px] w-[200px] items-center justify-center rounded-full overflow-visible"
    >
      {/* Breathing outer glow layers */}
      <motion.div
        animate={{
          opacity: [0.15, 0.4, 0.15],
          scale: [1, 1.15, 1],
        }}
        transition={{
          duration: 3,
          repeat: Infinity,
          ease: "easeInOut",
        }}
        className="absolute -inset-8 rounded-full bg-orange-500/30 blur-2xl"
      />

      <motion.div
        animate={{
          opacity: [0.1, 0.3, 0.1],
          scale: [1, 1.1, 1],
        }}
        transition={{
          duration: 3,
          repeat: Infinity,
          ease: "easeInOut",
          delay: 0,
        }}
        className="absolute -inset-4 rounded-full bg-amber-400/25 blur-xl"
      />

      {/* Outer shell with animated glow */}
      <motion.div
        animate={{
          boxShadow: [
            "0 0 40px rgba(249,115,22,0.4)",
            "0 0 80px rgba(249,115,22,0.7)",
            "0 0 40px rgba(249,115,22,0.4)",
          ],
        }}
        transition={{
          duration: 3,
          repeat: Infinity,
          ease: "easeInOut",
          delay: 0,
        }}
        className="absolute inset-0 rounded-full border border-white/30"
      />

      {/* Core SVG with radial gradient and rotating rings */}
      <div className="absolute inset-0 flex items-center justify-center">
        <svg width="190" height="190" viewBox="0 0 190 190">
          <defs>
            <radialGradient id="orangeGlow" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stopColor="rgba(251,146,60,0.35)" />
              <stop offset="50%" stopColor="rgba(249,115,22,0.2)" />
              <stop offset="100%" stopColor="rgba(234,88,12,0.08)" />
            </radialGradient>
          </defs>
          <circle cx="95" cy="95" r="88" fill="url(#orangeGlow)" />
          
          {/* Rotating dashed rings with dynamic styling */}
          {[0, 3, 6].map((offset, i) => (
            <circle
              key={i}
              cx="95"
              cy="95"
              r={83 + offset}
              fill="none"
              stroke={`rgba(${251 - i * 5},${146 - i * 20},${60 + i * 20},${0.5 - i * 0.12})`}
              strokeWidth={2.5 - i * 0.5}
              strokeDasharray={`${70 + i * 25} ${25 - i * 5}`}
              strokeLinecap="round"
              style={{
                animation: `spin ${3 + i * 1.5}s linear infinite`,
                transformOrigin: "95px 95px",
              }}
            />
          ))}
          
          {/* Inner pulsing ring */}
          <circle
            cx="95"
            cy="95"
            r="72"
            fill="none"
            stroke="rgba(251,146,60,0.6)"
            strokeWidth="2"
            className="animate-pulse"
          />
          <style>
            {`
              @keyframes spin {
                from { transform: rotate(0deg); }
                to { transform: rotate(360deg); }
              }
            `}
          </style>
        </svg>
      </div>

      {/* J.A.R.V.I.S text with stretched appearance */}
      <h1 className="z-10 text-white text-center select-none" style={{ 
        fontWeight: 900, 
        fontSize: '15px',
        letterSpacing: '0.25em',
        lineHeight: 1,
        transform: 'scaleY(2.4)',
        textIndent: '0.25em',
        paddingLeft: '0.15em',
      }}>
        J.A.R.V.I.S
      </h1>
    </motion.div>
  );
}